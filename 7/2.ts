...
const getHandType = (hand: string): HandTypes | 'test' => {
  ...
  const jockerOccurences = cardOccurenceMapping.get('J');

  if (jockerOccurences && cardOccurenceMapping.size >= 1) {
    if (cardOccurenceMapping.size === 1) {
    } else {
      cardOccurenceMapping.delete('J');
      let highestKey: string | undefined = undefined;
      for (let [card, occurence] of cardOccurenceMapping) {
        if (!highestKey) {
          highestKey = card;
        } else {
          const _highestVal = cardOccurenceMapping.get(highestKey);
          if (_highestVal && _highestVal < occurence) {
            highestKey = card;
          }
        }
      }
      if (highestKey) {
        cardOccurenceMapping.set(
          highestKey,
          (cardOccurenceMapping.get(highestKey) || 0) + jockerOccurences
        );
      }
    }
  }
